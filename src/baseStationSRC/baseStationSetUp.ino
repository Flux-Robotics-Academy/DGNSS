//This Script is used to determine the location of a base station using PPP
  //Gather RAW GNSS DATA
  //Processing using a government PPP tool
    //AUSPOS or CSRS-PPP (Canada)
  //This returns ECEF data which can then be input into BaseStationFunction script
//5th December 2022

//TO FINE TUNE --> Look at Example23_getRXMRAWX
#include <SPI.h>
#include <SD.h>
...
SFE_UBLOX_GNSS baseStation;
    ...
    File surveyData; //File that all GNSS data is written to
...
void setup(){
  ...
  // On the Artemis, we can disable the internal I2C pull-ups too to help reduce bus errors
    ...
    surveryDATA = SD.open("RXM_RAWX.ubx", FILE_WRITE);
    if(!surveyData)
    {
    ...
    //baseStation.enableDebugging(); // Uncomment this line to enable lots of helpful GNSS debug messages on Serial
    //baseStation.enableDebugging(Serial, true); // Or, uncomment this line to enable only the important GNSS debug messages on Serial

    baseStation.disableUBX7Fcheck(); // RAWX data can legitimately contain 0x7F, so we need to disable the "7F" check in checkUbloxI2C
    ...
    baseStation.setFileBufferSize(fileBufferSize); // setFileBufferSize must be called _before_ .begin

    if (baseStation.begin() == false) //Connect to the u-blox module using Wire port
    {
    ...
    baseStation.setI2COutput(COM_TYPE_UBX); //Set the I2C port to output UBX only (turn off NMEA noise)
    baseStation.saveConfigSelective(VAL_CFG_SUBSEC_IOPORT); //Save (only) the communications port settings to flash and BBR

    baseStation.setNavigationFrequency(1); //Produce one navigation solution per second (that's plenty for Precise Point Positioning)

    baseStation.setAutoRXMSFRBXcallbackPtr(&newSFRBX); // Enable automatic RXM SFRBX messages with callback to newSFRBX

    baseStation.logRXMSFRBX(); // Enable RXM SFRBX data logging

    baseStation.setAutoRXMRAWXcallbackPtr(&newRAWX); // Enable automatic RXM RAWX messages with callback to newRAWX

    baseStation.logRXMRAWX(); // Enable RXM RAWX data logging
...
void loop(){
  ...
  baseStation.checkUblox(); // Check for the arrival of new data and process it.
  baseStation.checkCallbacks(); // Check if any callbacks are waiting to be processed.

  // =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-

  while (baseStation.fileBufferAvailable() >= sdWriteSize) // Check to see if we have at least sdWriteSize waiting in the buffer
  {
    digitalWrite(LED_BUILTIN, HIGH); // Flash LED_BUILTIN each time we write to the SD card

    baseStation.extractFileBufferData(myBuffer, sdWriteSize); // Extract exactly sdWriteSize bytes from the UBX file buffer and put them into myBuffer

    surveryData.write(myBuffer, sdWriteSize); // Write exactly sdWriteSize bytes from myBuffer to the ubxDataFile on the SD card
  ...
  if (millis() > (lastPrint + 1000)) // Print the message count once per second
  {
    ...
    uint16_t maxBufferBytes = baseStation.getMaxFileBufferAvail(); // Get how full the file buffer has been (not how full it is now)
  ...
  {
    uint16_t remainingBytes = baseStation.fileBufferAvailable(); // Check if there are any bytes remaining in the file buffer
    ...
    while (remainingBytes > 0) // While there is still data in the file buffer
    {
      ...
      baseStation.extractFileBufferData(myBuffer, bytesToWrite); // Extract bytesToWrite bytes from the UBX file buffer and put them into myBuffer

      surveryData.write(myBuffer, bytesToWrite); // Write bytesToWrite bytes from myBuffer to the ubxDataFile on the SD card
    ...
    surveryData.close(); // Close the data file
