...
void setup()
{
  ...
  bool response = true;
  response &= myGNSS.enableRTCMmessage(UBX_RTCM_1005, COM_PORT_UART2, 1); //Enable message 1005 to output through I2C port, message every second
  response &= myGNSS.enableRTCMmessage(UBX_RTCM_1074, COM_PORT_UART2, 1);
  response &= myGNSS.enableRTCMmessage(UBX_RTCM_1084, COM_PORT_UART2, 1);
  response &= myGNSS.enableRTCMmessage(UBX_RTCM_1094, COM_PORT_UART2, 1);
  response &= myGNSS.enableRTCMmessage(UBX_RTCM_1124, COM_PORT_UART2, 1);
  response &= myGNSS.enableRTCMmessage(UBX_RTCM_1230, COM_PORT_UART2, 10); //Enable message every 10 seconds
  bool response1 = true;
  response1 &= myGNSS.enableRTCMmessage(UBX_RTCM_1005, COM_PORT_I2C, 1); //Enable message 1005 to output through I2C port, message every second
  response1 &= myGNSS.enableRTCMmessage(UBX_RTCM_1074, COM_PORT_I2C, 1);
  response1 &= myGNSS.enableRTCMmessage(UBX_RTCM_1084, COM_PORT_I2C, 1);
  response1 &= myGNSS.enableRTCMmessage(UBX_RTCM_1094, COM_PORT_I2C, 1);
  response1 &= myGNSS.enableRTCMmessage(UBX_RTCM_1124, COM_PORT_I2C, 1);
  response1 &= myGNSS.enableRTCMmessage(UBX_RTCM_1230, COM_PORT_I2C, 10); //Enable message every 10 seconds
  ...
  else
  {
    ...
    response = myGNSS.enableSurveyMode(60, 2.000); //Enable Survey in, 60 seconds, 5.0m
  ...
  myGNSS.setUART2Output(COM_TYPE_UBX | COM_TYPE_RTCM3); //Set the I2C port to output UBX and RTCM sentences (not really an option, turns on NMEA as well)
}
